#!/usr/bin/env python3
"""
Valid ROS2 Node Example - Demonstrates proper ROS2 conventions and best practices.

This example shows a well-structured ROS2 node that follows all best practices:
- Proper imports and node structure
- Parameter declaration and handling
- Publisher and subscriber with QoS settings
...
class ValidRos2Node(Node):
    """
    A well-structured ROS2 node demonstrating proper conventions and best practices.

    This node implements:
    - Proper parameter handling
    - Publisher with appropriate QoS
    ...
    def __init__(self):
        ...
        super().__init__('valid_ros2_node')
        ...
        self.get_logger().info(
            f'Valid ROS2 Node initialized with frequency: {self.publish_frequency}Hz, '
    ...
    def timer_callback(self) -> None:
        ...
        try:
            ...
            msg = String()
            msg.data = f'Valid ROS2 Node message #{self.message_counter} at {time.time():.2f}'
...
def main(args=None):
    ...
    try:
        ...
        valid_ros2_node = ValidRos2Node()

        # Create multi-threaded executor to handle callbacks concurrently
        executor = MultiThreadedExecutor(num_threads=4)
        executor.add_node(valid_ros2_node)

        try:
            # Spin the node to process callbacks
            valid_ros2_node.get_logger().info('Starting valid ROS2 node...')
            executor.spin()
        except KeyboardInterrupt:
            valid_ros2_node.get_logger().info('Keyboard interrupt received')
        finally:
            # Cleanup
            executor.shutdown()
            valid_ros2_node.destroy_node()
